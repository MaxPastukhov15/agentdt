...
class VectorRepository:
    ...
    def __init__(self, location_to_save: str, collection_name: str) -> None:
        ...
        self.vector_store: QdrantVectorStore = QdrantVectorStore(embedding=self.embeddings,
                                     collection_name = self.collection_name, 
                                     client=self.client)
        
        self.text_splitter: RecursiveCharacterTextSplitter = RecursiveCharacterTextSplitter(chunk_size=1024,
                                                        chunk_overlap=204, length_function=len, is_separator_regex=False,)
    ...
    def _load_pdf(self, file_name: Path)->List[Document]:
        ...
        loader = PyMuPDF4LLMLoader(file_path=file_name, table_strategy=None, 
                                   ignore_graphics=True, ignore_images=True, mode='page')
    ...
    def __close(self):
        ...
        if hasattr(self, 'client') and self.client:
            ...
            logger.error(f"Client doesn't exists")